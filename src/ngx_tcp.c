...
static ngx_int_t ngx_tcp_optimize_servers(ngx_conf_t *cf, 
...
static ngx_listening_t *ngx_tcp_add_listening(ngx_conf_t *cf, ngx_tcp_conf_addr_t *addr);
...
ngx_tcp_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_tcp_conf_ctx_t **) conf = ctx;
    ...
    /*
     * create the main_conf's, and the null srv_conf's
    ...
    *cf = pcf;
    ...
    if (ngx_tcp_optimize_servers(cf, cmcf, &ports) != NGX_OK) {
        return NGX_CONF_ERROR;
    }
